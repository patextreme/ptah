...
#[test]
fn a_duplicate_alias_with_an_alias_form_override_is_a_usage_error() {
    // Regression: with a lockfile whose name/version/target match,
    // check_lockfile resolves overrides before any other
    // duplicate-alias check — the unwrap inside resolve_overrides
    // panicked (exit 101). Contract: usage error naming the alias,
    // exit 2, never a panic.
    let p = Project::new("dup-alias");
    std::fs::write(
    ...
    .unwrap();
    std::fs::write(
        p.dir.join(".ptah/pesde.lock"),
        "# This file is automatically @generated by pesde.\n# It is not intended for manual editing.\nformat = 2\nname = \"abc/x\"\nversion = \"0.1.0\"\ntarget = \"luau\"\n",
    )
    .unwrap();
    for args in [vec!["install"], vec!["install", "--locked"], vec!["update"]] {
        let (code, _out, stderr) = p.package(&args);
        assert_eq!(code, 2, "args {args:?}: stderr:\n{stderr}");
        assert!(stderr.contains("duplicate dependency alias"), "{stderr}");
