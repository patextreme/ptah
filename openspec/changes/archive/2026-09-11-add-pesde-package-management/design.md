...
`Project::new(package_dir = <project>/.ptah, workspace_dir = None, data_dir = <project>/.ptah/.pesde/.data, cas_dir = <project>/.ptah/.pesde/.cas, AuthConfig::new())`. Project discovery: walk up from the invocation dir for a directory containing `.ptah/config.toml` or `.ptah/pesde.toml` (generalizing `ptah-config::find_project_config`, which only looks for `config.toml`). Per-project caches (not `~/.pesde`) keep ptah invisible to a user's own pesde install and make the sandbox/nix story simple; CAS sits under `.ptah` so the same-filesystem hard-link constraint holds trivially. The subdirectory names are dot-prefixed (`.data`/`.cas`) on purpose, discovered during implementation: pesde's post-install `remove_unused` sweep treats `<package>/.pesde` as its scripts-link folder and deletes entries whose names parse as Pesde aliases but aren't graph aliases — plain `data`/`cas` parse as aliases and would be wiped on every install; the dotted names don't parse and survive. Trade-off: no cross-project cache sharing (bigger disks, re-downloads across projects) — acceptable, revisit if users complain; a future `data_dir` under `$XDG_CACHE_HOME` is a config knob away.

### D4 — Default index supplied in-memory, manifest stays textual
The skeleton carries no `[indices]`. At resolve time the adapter deserializes the manifest and, when `indices` is empty, injects `default = <ptah's compiled-in default index URL>` (pesde's own default index) into the in-memory `Manifest` before resolution; writes never go through `Manifest` serialization (`indices` is `skip_serializing` anyway) but through `toml_edit` on the raw text, so user `[indices]` entries survive byte-for-byte. Implementation revealed one wrinkle: pesde's `dependency_graph` **re-reads the manifest from disk**, so the in-memory injection alone never reaches library resolution. The adapter therefore materializes the default index into the on-disk manifest (toml_edit) around each resolution pass and restores the user's original bytes afterwards — the committed manifest never keeps a ptah-written `[indices]` table, and a crash mid-operation leaves at worst a table naming the same URL ptah would inject (self-healing, identical semantics). `PTAH_DEFAULT_INDEX` overrides the compiled-in URL — the air-gapped escape hatch and the hook the offline test suite uses to point the default at its loopback fixture. Default URL is a `const` in `ptah-pesde` — one-line change if ptah ever hosts its own index.
...
`ScriptRequier` flips `has_config()` to true (a `.luaurc`/`.config.luau` exists in the current directory) and `config()` returns its bytes — mlua then performs the walk-up and JSON parsing itself and calls `jump_to_alias` with the resolved path, which we land like any other path. Standard Luau semantics for free; no ptah-specific alias table in the runtime. `ptah-check`'s lint walker duplicates the walk-up purely (walk parents from the requiring file, parse `aliases`, resolve relative to the config's directory), the same deliberate duplication pattern as today's `resolve_edge`. One implementation-driven addition: the `--!strict` lint exempts files under a `luau_packages/` directory — pesde's linker modules are machine-generated without the directive, and the package-management spec's "installed packages check clean" scenarios require them not to be findings; the directive remains required for every authored file. The luau-lsp pass: verified during the spike — `luau-lsp analyze` discovers the project-root `.luaurc` from the entry's path and resolves `@alias` requires through it (no `--base-luaurc` needed).
