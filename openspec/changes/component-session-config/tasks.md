...
- [ ] 1.1 Create `factory-components/std/session-config.luau` (`--!strict`):
...
- [ ] 2.1 Replace `PredicateOptions.model` with
...
- [ ] 2.2 Add a judge-propagation scenario: entries configured on a
...
- [ ] 3.1 `factory-components/components/openspec/component.luau`: replace
...
- [ ] 3.3 Migrate the existing component test shims in
      ...
      `cargo test --test factory_components` is green
- [ ] 3.4 Add component scenarios pinning propagation: openspec work +
...
- [ ] 3.5 Add compatibility-gate scenarios (extend
...
- [ ] 4.1 Migrate `.ptah/workflows/openspec/main.luau`,
...
- [ ] 5.1 `factory-components/README.md`: add `session-config` to the std
...
- [ ] 5.2 Both component READMEs: document `sessionConfig`/
...
- [ ] 5.3 `CONTEXT.md`: add the **Session config** entry (the ordered list
...
- [ ] 6.1 `cargo test` (full suite) green inside `nix develop`
- [ ] 6.2 `stylua .` clean and `ptah check` green on every edited `.luau`
