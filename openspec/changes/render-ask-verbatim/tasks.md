...
- [ ] 1.1 Rework ask rendering in `crates/ptah-render/src/lib.rs`: replace
      ...
      a 2-space-indented line, trailing blank lines trimmed, interior blank
      lines rendered empty (no indent padding) — no collapse, no
      `truncate_visible` on any ask line. Update `ask_requested` to emit
      the lines through the existing timestamped `ask_line` path with the
      `> ` cue last. Verify the renderer unit tests below pass.
- [ ] 1.2 Update/replace the renderer unit tests that pinned the old shape:
      ...
      blank line rendering empty, trailing-newline trimming, cue emitted
      after the last prose line. Verify with `cargo test -p ptah-render`.

## 2. Integration and gates

- [ ] 2.1 Confirm no other surface pinned the old shape: run the full suite
...
- [ ] 2.2 Exercise the real binary against a long multi-line ask once by
